...
class ArgoverseRoadDetectionLoader:
    def __init__(self, root_dir: str , av_hd_map_dir = "../map_files/" , max_number_of_log = None , LIST_OF_LOG_FOR_VISUALIZATION : List[ str ] = None , range_drivable_area_map = None ) -> None:
        ...
        self._bev_tensor_list_ : Optional[ Dict[ str , List[ str ]]] = None
    ...
    def lidar_list(self) -> List[str]:
        ...
        if self._lidar_list is None:
            ...
            self._lidar_list = self.all_bev_tensor_list()
        ...
        if self._lidar_list is None:
            ...
            for log in self.log_list:
                ...
                if self.LIST_OF_LOG_FOR_VISUALIZATION is not None :
                    ...
                    # Then take only one frame for visualization in all log 
    ...
    def all_bev_tensor_list(self) -> Dict[str , List[str]]:
        ...
        for log in self.log_list:
            ...
            self._bev_tensor_list_[log] = sorted(glob.glob(os.path.join(self.root_dir, log, "BEV_tensor_folder", "*.pickle")))

            if self.LIST_OF_LOG_FOR_VISUALIZATION is not None :

                self._bev_tensor_list_[ log ] = self._bev_tensor_list_[ log ][ 0 : 1 ]

            #self._bev_tensor_list_[ log ] = [str( i ).replace( ".pickle" , '.ply') for i in self._lidar_list[ log ]]

            #print( "Directory of BEV tensor is : " + str( os.path.join(self.root_dir, log, "BEV_tensor_folder", "*.pickle") ))

            self.bev_tensor_count += len(self._bev_tensor_list_[log])

        
        if self.bev_tensor_count > self.lidar_count :
            # Then change all LiDAR list based on BEV tensor drivable area folder

            for log in self.log_list :

                self._lidar_list[ log ] = [ str(i).replace("BEV_tensor_folder" , "lidar").replace( ".pickle" , ".ply") for i in self._bev_tensor_list_[ log ] ]
    ...
    def _ensure_lidar_timestamp_list_populated(self) -> None:
        ...
        if self._lidar_timestamp_list is None:
            ...
            #print( "Argoverse dataset LiDAR list is : " + str( self._lidar_list ))
            for log in self.log_list:
                self._lidar_timestamp_list[log] = [
                    int(x.split("/")[-1][:-4].split("_")[-1]) for x in self._lidar_list[log]
                ]

                #print( "Timestamp list of lidar in log : {} have number frame : {}".format( log , len( self._lidar_timestamp_list[ log ])))
    ...
    def __next__(self , av_hd_map_dir = "../map_files/" ) -> "ArgoverseTrackingLoader":
        self.counter += 1
...
Total bounding box: {sum(num_annotations)}
    ...
    def get_rasterized_drivabel_area_label( self , key : int , map_range : list = [ -50 , -50 , 50 , 70] , grid_size : list = [0.2, 0.2]) -> np.array :
        ...
        length_of_bev_map = int((map_range[ 2 ] - map_range[ 0 ])/grid_size[0])
        width_of_bev_map = int((map_range[3] - map_range[1])/grid_size[1])

        img = img.resize( ( length_of_bev_map, width_of_bev_map ) , resample=Image.BOX)
