...
class Calibration:
    ...
    def __init__(self, camera_config: CameraConfig, calib: Dict[str, Any]) -> None:
        ...
        self.inverse_projection_image_to_lidar_points = np.linalg.pinv( self.K.dot( self.extrinsic ))

        # Make attribute inverse projection image to lidar points to project DA on image to BEV

        assert self.inverse_projection_image_to_lidar_points.shape[ -2 : ] == (4,3)
        if self.inverse_projection_image_to_lidar_points[2][2] == 0 :

            print( "Value of inverse_projection_image_to_lidar_points on z coordinate is 0" )

            self.inverse_projection_image_to_lidar_points[2][2] = 1e-6
    ...
    def find_image_depth_for_known_height( self , image_coordinate : np.array , known_height : float = -1.5 ) :
        
        """ By using equation :

        xyz_ego = inverse_matrix( extrinsic * intrinsic )*xy_image

        [ x , y , z , 1 ] = self.inverse_projection_image_to_camera * [x , y , image_depth]

        That means z = self.inverse_projection_image_to_camera[ 2 , 0 ] * x + self.inverse_projection_image_to_camera [ 2 , 1 ] * y + self.inverse_projection_image_to_camera [2][2] * image_depth

        That means image_depth = ( z -  self.inverse_projection_image_to_camera[ 2 , 0 ] * x  - self.inverse_projection_image_to_camera [ 2 , 1 ] * y )/( self.inverse_projection_image_to_camera [2][2] )
        """

        return( ( known_height - self.inverse_projection_image_to_lidar_points[2,0] * image_coordinate[0] - self.inverse_projection_image_to_lidar_points[2,1]* image_coordinate[1])/ self.inverse_projection_image_to_lidar_points[2][2] )

    def project_DA_image_coordinate_to_lidar_coordinate( self , image_coordinate : np.array , known_height : float = -0.98 ) :

        # Find image dept of drivable area in image coordinate

        image_depth = self.find_image_depth_for_known_height( image_coordinate , known_height )

        return( self.project_image_to_ego( np.append( image_coordinate , np.array( [ image_depth ] ) , axis = 0 ).reshape( 1, 3 , 1 )))
